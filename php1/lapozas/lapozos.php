...
function menuPont($szoveg, $tartalomId, $aktivMenu, $enabled)
{
	if ($enabled) {
		...
		return '<li class="page-item' . ($aktivMenu ? " disabled" : "") . '"><a class="page-link">' . $szoveg . '</a></li>';
	}
}

$menu = "";
$menu .= menuPont("Előző", isset($_GET["oldal"]) ? $_GET["oldal"] - 1 : 1, false, isset($_GET["oldal"]) ? $_GET["oldal"] != 1 : false);

for ($i = 1; $i <= 10; $i++) {
	$menu .= menuPont($i, $i, (isset($_GET["oldal"]) && $_GET["oldal"] == $i) || (!isset($_GET["oldal"]) && $i == 1), true);
}

$menu .= menuPont("Következő", isset($_GET["oldal"]) ? $_GET["oldal"] + 1 : 2, false, isset($_GET["oldal"]) ? $_GET["oldal"] != 10 : true);
